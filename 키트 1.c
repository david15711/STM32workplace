...
void SysTick_Handler(void)			/* SysTick interrupt function */
{
  ...
  if(100 - (second / 100) == 0 ){
   SysTick->CTRL  = 0x00000001;// interrupt disable
     TFT_string(30, 14, Black, Green, " 심정지 " );
     TX3_char(0x0D); TX3_char(0x0A);
     TX3_string(" Heartrate is stop ");
     TX3_char(0x0D); TX3_char(0x0A);
  }
  
  if(second % 20 == 0){
     TFT_xy(30,12);
     TFT_unsigned_decimal(100 - (second / 100),1,2);
     TFT_string_large(0, 8, Black, Green, " 구조대상자의 건강상태");
     TFT_string(0, 12, Black, Green, " 구조대상자의 심박수 : " );
      }
}
...
void EXTI0_IRQHandler(void)			/* EXTI0 interrupt function */
{
    /*여기에 key 1 기능을 적어주세요
	zone 1에 재난 발생*/
    TFT_string_large(0, 4, Black, Green, " Zone 1 ");
     TX3_string(" Zone 1 emergency ");
     TX3_char(0x0D); TX3_char(0x0A);
...
void EXTI1_IRQHandler(void)			/* EXTI1 interrupt function */
{
    /*여기에 key 2 기능을 적어주세요
	zone 2에 재난 발생*/
     TFT_string_large(9, 4, Black, Green, " Zone 2 ");
     TX3_string(" Zone 2 emergency ");
     TX3_char(0x0D); TX3_char(0x0A);
...
int main(void) {
    ...
    SysTick->LOAD  = 139999;			// 27MHz/(269999+1) = 100Hz
