/* ============================================================================ */
/*	Exp21_2_loopback(2).c : CAN 2.0B(29비트 ID를 사용) 루프백 테스트	*/
/* ============================================================================ */
...
int main(void)
{
  ...
  TFT_string(10,3,White,Black,"CAN 2.0B(29비트 ID)");
...
void CAN1_initialize(void)			/* initialize bxCAN CAN1 */
{
  ...
  CAN1->sFilterRegister[0].FR1 = 0x00000004;	// set ID of filter number 0 and IDE=1
  CAN1->sFilterRegister[0].FR1 |= (TX_ID << 3);
...
void CAN1_transmit(U32 ID, U32 DLC)		/* transmit CAN1 message with ID and data length */
{
  ...
  CAN1->sTxMailBox[TX_empty].TIR = 0x00000004;	// IDE=1, RTR=0
  CAN1->sTxMailBox[TX_empty].TIR |= (ID << 3);	// 29-bit ID(CAN 2.0B)
